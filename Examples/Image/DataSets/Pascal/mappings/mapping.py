...
def create_mappings(abs_path, train, skip_difficult):
        ...
        with open(roi_file_path, 'w') as roi_file:
            with open(size_file_path, 'w') as size_file:
                for in_line in input_lines:
                    ...
                    annotation_file = os.path.join(abs_path, pascal_voc2007_annotations_rel_path, "{}.xml".format(img_number))
                    ...
                    with Image.open(os.path.join(abs_path, img_file_path)) as img:
...
def create_pascal_mappings(abs_path):
    create_mappings(abs_path, True, skip_difficult=True)
    create_mappings(abs_path, False, skip_difficult=True)
    create_mappings(abs_path, True, skip_difficult=False)
    create_mappings(abs_path, False, skip_difficult=False)

if __name__ == '__main__':
    create_pascal_mappings(abs_path)
